...
# load relevant packages - using pacman
if (!require('pacman')) {
...
```{r, echo = FALSE}
# left join the team dataset and the player dataset 
# select distinct rows (there are duplicates)
...
# filter and sort based on article details
nba_gen <- nba %>% 
 ...
 raptor_offense, raptor_defense, raptor_defense, raptor_total, war_total) %>% 
  filter(mp >= 1137) %>% 
  arrange(-war_total)
# this code chunk created the final dataframe and everything after just formats it/turns it into a graph
```
...
```{r, echo = FALSE}
#create the grouped columns for reproducing the table
...
```{r, echo = FALSE}
# rename columns to match table from article: 
nba_gen <- nba_gen %>% 
  rename(
    PLAYER = player_name,
    TEAM = team,
...
# get the unabbreviate version of all the nba team names
...
# left join the team names to the dataset
nba_gen <- nba_gen %>% 
  left_join(select(nba_teams, TEAM, full_name), by = "TEAM")

# select all the columns but replace the team name with the unabbreviated version
# rename team column
...
# reformat the minutes played column to include a comma
nba_gen$MINUTES <- format(nba_gen$MINUTES,big.mark=", ", trim=TRUE)
# reformat the season column to only include the last 2 digits of the year (2023 to 23)
...
```{r, echo = FALSE}
# this code chunk creates the reactable for recreating the table in the article

# create color palette for table
blue_pal <- function(x) rgb(colorRamp(c("#fa8072", "white", "#35b0ab"))(x), maxColorValue = 255)

table <- reactable(
  nba_gen,
  columns = list(
    #this first part adds the season to the player column 
    PLAYER = colDef(
    ...
    raptor_offense = colDef(style = function(value) {
      ...
      headerStyle = list(fontWeight = "100",
                         fontSize = "0.75rem")),
    # these next few chunks reformat the columns to add color based on the values
    # also adds a + in front of positive values
  ...
  # this next chunk groups the columns under certain headers
  columnGroups = list(
...
```{r, echo = FALSE}
# this code chunk creates the graph from the article
# the part that says text = creates the labels for each data point in the graph
fig <- nba_gen %>% 
  ...
  # the layout section creates and formats the axes labels
  layout(title = "Where every player in the NBA stands, according to RAPTOR",
         ...
         yaxis = list(title = "Defensive RAPTOR rating",
                      ...
                      zerolinecolor = "lightgrey",
                      gridcolor = "lightgrey")) 
# the following code adds multiple annotations to add in the offense/defense indicators in each quadrant
...
# these next couple code chunks color in the top right and bottom left quadrants of the graph
fig <- fig %>% layout(
  shapes = list(
    list(
      type = "rect",
